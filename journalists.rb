...
print "Combien d'handle contiennent un numéro ? : " #379
puts handles.size

print "Combien d'handle contiennent un numéro ? : " #17
counter = 0
handles.each { |handle|
  if handle =~ /\d/
    counter += 1
...
print "Combien d'handle contiennent les 4 lettres du prénom Aude à la suite (sans faire attention à la casse) ? : " #3
counter = 0
handles.each { |handle|
...
print "Combien commencent par une majuscule ? : " #209
counter = 0
handles.each { |handle|
  if handle =~ /^@[A-Z]/
    #puts handle
...
print "Combien contiennent une majuscule ? : " #215
counter = 0
handles.each { |handle|
  if handle =~ /[A-Z]/
    #puts handle
...
print "Combien de _ dans tous les pseudos confondus ? : " #75
sum = 0
handles.each { |handle|
  #puts "#{handle}: #{handle.count('_')}"
  sum += handle.count('_')
}
puts sum
